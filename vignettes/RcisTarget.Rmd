...
library(data.table)
#require(visNetwork)
# library(doMC);library(doRNG) # Loaded by the package, to avoid messages. Not available in windows.
})
```
...
Species | Genome | | Package
------- | ----- | ----- | -----
Human | hg19 | Full version (recommended) | [SCENIC.hg19.motifDatabases.20k](http://scenic.aertslab.org/downloads/databases/RcisTarget.hg19.motifDatabases.20k_0.1.1.tar.gz)
Human | hg19 | Subset | - available soon: *RcisTarget.hg19.motifDatabases.cisbpOnly.500bp*
Mouse | mm9 | Full version (recommended) | [SCENIC.mm9.motifDatabases.20k](http://scenic.aertslab.org/downloads/databases/RcisTarget.mm9.motifDatabases.20k_0.1.1.tar.gz)
Mouse | mm9 | Subset | - available soon: *RcisTarget.mm9.motifDatabases.cisbpOnly.500bp*
Drosophila melanogaster | dm6 | | Contact us
...
In addition to the full versions of the databases (20k motifs), we also provide a subset containing only the 4.6k motifs from cisbp. These subsets are available in Bioconductor for demonstration purposes. They will provide the same AUC score for the existing motifs. However, we highly recommend to use the full version (20k) for more accurate results, since the normalized enrichment score (NES) of the motif depends on the total number of motifs in the database.
...
### Gene-motif rankings
For most species, there are two **gene-motif rankings** available: **10kb around the TSS** or **500 bp upstream**, which determine the search space around the transcription start site (TSS). To see the names of the rankings and annotations for each species, you can type on your console: `?RcisTarget.mm9.motifDatabases.20k` or `?RcisTarget.hg9.motifDatabases.20k`. 
...
*(Change the prefix to the relevant species, i.e. mm9_500bpUpstream_motifRanking)*

```{r installDatabases, eval=FALSE}
# To download the rankings:
source("http://bioconductor.org/biocLite.R")
biocLite("RcisTarget.hg19.motifDatabases.20k")
```

Example for loading the databases: 
```{r loadDatabases, eval=FALSE}
# Full version (20k motifs) for human
library(RcisTarget.hg19.motifDatabases.20k)
# Search space: 10k bp around TSS
data(hg19_10kbpAroundTss_motifRanking)

# The db can be saved as a variable for easy substitution in the following steps:
motifRankings <- hg19_10kbpAroundTss_motifRanking
motifRankings
```

For this example (demonstration purposes), we will use the subset of the database:
...
### Motif annotations
All the calculations performed by RcisTarget are motif-based. However, most users will be interested on TFs potentially regulating the gene-set. The association of motif to transcription factors are provided in two databases: one containing the TFs **directly** annotated to the motif (i.e. in the source motif database), and **inferred** annotation based on motif similarity.

These databases dan be provided as `motifAnnot_direct` or `motifAnnot_inferred` arguments to `cisTarget()` or `addMotifAnnotation()`.

Database| Motif-TF annotation
------- | -----
`hg19_direct_motifAnnotation` | Direct 
`hg19_inferred_motifAnnotation` | Inferred by similarity 

*(Change the prefix to the relevant species, i.e. `mm9_500bpUpstream_motifRanking` or add the sufix `_cisbpOnly` if appropiate)*
...
motifEnrichmentTable <- addMotifAnnotation(motifs_AUC, nesThreshold=3, 
                     motifAnnot_direct=hg19_direct_motifAnnotation_cisbpOnly,
                     motifAnnot_inferred=hg19_inferred_motifAnnotation_cisbpOnly,
...
head(motifEnrichmentTable[,-"TF_inferred", with=FALSE])
```
