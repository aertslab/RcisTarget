...
------- | ----- | ----- | -----
Human | hg19 | Full version (recommended) | [RcisTarget.hg19.motifDBs.20k](http://scenic.aertslab.org/downloads/databases/RcisTarget.hg19.motifDBs.20k_0.2.0.tar.gz)
Mouse | mm9 | Full version (recommended) | [RcisTarget.mm9.motifDBs.20k](http://scenic.aertslab.org/downloads/databases/RcisTarget.mm9.motifDBs.20k_0.2.0.tar.gz) 
...
Species | Genome | | Package
------- | ----- | ----- | -----
Human | hg19 | Subset | - available soon: *RcisTarget.hg19.motifDBs.cisbpOnly.500bp*
Mouse | mm9 | Subset | - available soon: *RcisTarget.mm9.motifDBs.cisbpOnly.500bp*


Example to download & install the packages:
...
# (For easy substitution in the following steps):
motifRankings <- hg19_10kbpAroundTss_motifRanking
motifRankings
```
...
### Motif annotations
All the calculations performed by RcisTarget are motif-based. However, most users will be interested on TFs potentially regulating the gene-set. The association of motif to transcription factors are provided in two databases: one containing the TFs annotated to each motif. In addition to the motifs annotated by the source datatabse (i.e. **direct** annotation), we have also **inferred** some further annotations based on motif similarity and gene ortology (e.g. similarity to other genes annotated to the motif).

These annotations are separate objects within each database package (typically containing "motifAnnotation" in their name), and are typically used with the functions `cisTarget()` or `addMotifAnnotation()`. For example:
```{r motifAnnotExample}
library(RcisTarget.hg19.motifDBs.cisbpOnly.500bp)
data(hg19_motifAnnotation_cisbpOnly) 
# motifEnrichTable <- addMotifAnnotation(.. ,motifAnnot=hg19_motifAnnotation)
hg19_motifAnnotation_cisbpOnly[199:202,]
```

These annotations can be easily queried to obtain further information about specific motifs or TFs:
```{r motifAnnotQuery}
hg19_motifAnnotation_cisbpOnly[(directAnnotation==TRUE) 
                               & 
                               (TF %in% c("TP53", "HIF1A")), ]
```
