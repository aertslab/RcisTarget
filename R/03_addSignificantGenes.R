...
#' @description Identify which genes (of the gene-set) are highly ranked for each motif.
#'
#' \itemize{
#'   \item addSignificantGenes(): adds them to the results table.
#'   \item getSignificantGenes(): Calculates the significant genes for ONE gene set. It provides the plot and the gene list (it is used by addSignificantGenes).
#' }
#' @param resultsTable [addSignificantGenes] Output table from \code{\link{addMotifAnnotation}}
#' @param geneSets [addSignificantGenes] List of gene-sets which was analyzed.
#' @param rankings Motif rankings used to analyze the gene list (They should be the same as used for calcAUC in this same analysis).
#' @param maxRank Maximum rank to take into account for the recovery curve (Default: 5000).
...
#' @details
#' The highly ranked genes are selected based on the distribution of the recovery curves of the gene set across all the motifs in the database.
#' In the plot, the red line indicates the average of the recovery curves of all the motifs, the green line the average + standard deviation, and the blue line the recovery curve of the current motif.
#' The point of maximum distance between the current motif and the green curve (mean+sd), is the rank selected as maximum enrichment. All the genes with lower rank will be considered enriched.
#'
#' Depending on whether the method is "iCisTarget" or "aprox", the mean and SD at each rank are calculated slightly different.
#' "iCisTarget" method calculates the recovery curves for all the motifs, and then calculates the average and SD at each rank.
#' Due to the implementation of the function in R, this method is slower than just subsetting the ranks of the genes in for each motif,
...
setGeneric("addSignificantGenes", signature="geneSets",
  function(resultsTable, geneSets, rankings, maxRank=5000, plotCurve=FALSE, genesFormat="geneList", method="aprox", nMean=20, nCores=1)
  {
...
setMethod("addSignificantGenes", "list",
  function(resultsTable, geneSets, rankings, maxRank=5000, plotCurve=FALSE, genesFormat="geneList", method="aprox", nMean=20, nCores=1)
  {
    .addSignificantGenes(resultsTable=resultsTable, geneSets=geneSets, rankings=rankings, maxRank=maxRank, plotCurve=plotCurve, genesFormat=genesFormat, method=method, nMean=nMean, nCores=nCores)
  })
...
.addSignificantGenes <- function(resultsTable, geneSets, rankings, maxRank=5000, plotCurve=FALSE, genesFormat="geneList", method="aprox", nMean=20, nCores=1)
{
  ...
  {
    enrRnkT_ByGs <- resultsTable[geneSet==gsn]

    signifGenes <- .getSignificantGenes(geneSet=as.character(geneSets[[unique(enrRnkT_ByGs$geneSet)]]),
                       rankings=rankings,
...
setGeneric("getSignificantGenes", signature="geneSet",
  function(geneSet, rankings, signifRankingNames=NULL, method="iCisTarget", maxRank=5000, plotCurve=FALSE, genesFormat=c("geneList", "incidMatrix"), nCores=1, digits=3, nMean=10)
  {
...
setMethod("getSignificantGenes", "list",
  function(geneSet, rankings, signifRankingNames=NULL, method="iCisTarget", maxRank=5000, plotCurve=FALSE, genesFormat=c("geneList", "incidMatrix"), nCores=1, digits=3, nMean=10)
  {
    ...
    .getSignificantGenes(geneSet=geneSet, rankings=rankings, signifRankingNames=signifRankingNames, method=method, maxRank=maxRank, plotCurve=plotCurve, genesFormat=genesFormat, nCores=nCores, digits=digits, nMean=nMean)
  })
...
.getSignificantGenes <- function(geneSet, rankings, signifRankingNames=NULL, method="iCisTarget", maxRank=5000, plotCurve=FALSE, genesFormat=c("geneList", "incidMatrix"), nCores=1, digits=3, nMean=10)
{
  ...
  if(method == "icistarget") {
    ...
    if(!"zoo" %in% rownames(utils::installed.packages())) stop("Package 'zoo' is required to calculate the aproximate RCC distributions. To install it, run:\t install.packages('zoo')")
  }
  ...
  {
    enrichedGenes[[rankingName]] <- sort(gSetRanks$rn[which(gSetRanks[,rankingName,with=FALSE] <= enrStats[rankingName, "rankAtMax"])])
  ...
  if(any(tolower(genesFormat) != "none")) return(ret)
}
