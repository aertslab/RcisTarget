...
#' @import data.table
.calcErn_iCisTarget <- function(gsRankings, maxRank, signifRankingNames, plotCurve, nCores)
{
...
#' @import data.table
.calcErn_Aprox <- function(gsRankings, maxRank, signifRankingNames, plotCurve, nCores)
{
  # Calculate aproximated-RCC across all motifs at each rank position 
  maxRankExtra <- maxRank+10
  ...
  rccStats <- t(apply(rccStatsRaw, 1, function(x) c(x[1:5], zoo::rollmean(x, 10, align="center", fill="extend"))))[,1:(maxRank-1)]
